...
def xlsxExport(data):
    workbook = xlsxwriter.Workbook('pages/zxspectrum-bom.xlsx')
    ...
    row = 0
    titles = ['Qty', 'Part', 'Remark', 'Package', 'Mouser', 'Components']
    widths = [   5 ,    50 ,      50 ,       10 ,      25 ,          60 ]
    ...
    for (title, sect) in data['bom'].items():
        ...
        for item in sect:
            row += 1
            worksheet.write(row, 0, item['quantity'])
            worksheet.write(row, 1, item['part'])
            if 'remark' in item:
                worksheet.write(row, 2, item['remark'])
            if 'package' in item:
                worksheet.write(row, 3, item['package'])
            if 'mouser' in item:
                mid = item['mouser']['id']
                mlink = 'https://www.mouser.com/ProductDetail/{}'.format(mid)
                worksheet.write_url(row, 4, mlink, string=mid)
            if 'components' in item:
                worksheet.write(row, 5, ' '.join(item['components']), fmtComponents)
...
with open("zxspectrum-bom.yml", 'r') as stream:
...
with open('pages/zxspectrum-bom.html', 'w') as out:
